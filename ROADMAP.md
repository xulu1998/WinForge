...
- **Status:** IN PROGRESS (Step 3.1 **COMPLETED** — real-desktop validation PASSED 2026-08-09; Step 3.2 **COMPLETED** — real-desktop validation PASSED 2026-08-09 on Windows 11 25H2 zh-CN x64 Consumer `install.wim`; Step 3.3 **IMPLEMENTED** on `feature/offline-customization` — **PENDING real-desktop validation**; no Step 3.4 / Phase 4 mount engine in this scope)
...
- **Status:** **IMPLEMENTED** on `feature/offline-customization` (sections A–Y complete; 206 automated tests pass — Core 37, App 169, 0 errors, 0 warnings, Release). **PENDING real-desktop validation** — no real Windows mount/registry/package mutation was exercised in this session. Not merged to `main`; no new tag.
...
- **Scope (this step):**
  ...
  - **Discovery services:** exact-identity provisioned-Appx enumeration (`DismAppxParser`, keyed by DISM "Deployment package name"), package enumeration with allowlist-gated `Removable` classification (`DismPackageParser`), offline registry via Win32 `RegLoadKey`/`RegUnLoadKey` under a WinForge-owned hive name (`OfflineRegistryService`, always unloaded in `finally`, ADR-023), service enumeration from the mounted image's `SYSTEM` hive, and a trusted `CustomizationDefinitionProvider` (5 Privacy + 3 System registry settings, 3 recommended service-start changes: DiagTrack/WerSvc/PcaSvc → Disabled). Discovery refuses to run when the mount session does not match (ADR-024).
