...
- **Status:** IN PROGRESS (Stage 11.1 **REAL DESKTOP VALIDATED** 2026-08-10; Stage 11.2 **IMPLEMENTED** 2026-08-10, **PENDING REAL DESKTOP REVIEW**; branch `phase/11-component-intelligence`, NOT merged to `main`)
...
- **Deliverables (Stage 11.2 — Component Knowledge Import + Catalog Expansion + Customize
  ...
  each carries provenance claims); and a Customize "Component Knowledge" tab
  (`ComponentKnowledgeViewModel`/`ComponentKnowledgeItem`/`ComponentKnowledgeView`) reusing
  `ComponentIntelligenceViewModel`'s classified inventory — human name/category/recommendation/risk
  badges, default usefulness sort, filters, compact hover quick card, click-for-detail without
  changing selection, conservative Protected/Unknown UX with explicit block reasons, official-vs-
  community evidence, deterministic "why" captions, **no automatic destructive selection**. Component
  Intelligence remains the advanced inspection surface. **530 automated tests pass (Core 53, App 477),
  0 errors, 0 warnings (Release).** ADR-045/ADR-046/ADR-047.
- **Acceptance Criteria (Stage 11.2):** catalog regenerated with no duplicate resx keys (idempotent
  ...
  RecommendedRemove; the Customize Component Knowledge tab renders with no XAML/binding crash; default
  sort places RecommendedRemove first then by risk/name; Standard mode hides raw Protected/Unknown
  details; clicking a component shows detail without changing the plan; no automatic destructive
  selection on load; build 0/0; all tests green.
- **Next (Stage 11.2 — PENDING REAL DESKTOP REVIEW):** Re-run the real Windows 11 25H2 zh-CN x64
  Consumer enumeration and a real-desktop validation pass of the Customize "Component Knowledge" tab
  (render, hover quick card, click-for-detail, sort/filter, official-vs-community evidence, no
  automatic destructive selection) before marking Phase 11 COMPLETE. Raw Windows identities remain
  ...
  consider Stage 11.3 (proposed): wire the Component Knowledge tab into the actual CustomizationPlan
  selection model so a user's keep/remove decision from the knowledge surface flows into the plan with
  explicit, provenance-backed confirmation — see DECISIONS ADR-047. Phase 11 remains IN PROGRESS; NOT
  merged to `main`.
