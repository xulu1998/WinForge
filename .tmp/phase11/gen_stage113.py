...
SHARED = [
    ...
    ("Profile.Summary.Adopt", "Suggested to apply", "建议执行"),
    ...
    ("Profile.Reapply", "Reapply recommendations", "重新采用推荐"),
    ("Profile.Restore", "Restore this configuration's recommendations", "恢复此配置推荐"),
    ("Profile.Details", "View recommendation details", "查看推荐详情"),
    ("Profile.BackToCustomize", "Back to Customize", "返回自定义"),
    ("Profile.Origin.Manual", "Manually selected", "手动选择"),
    ("Profile.Origin.Auto", "Auto-selected by {0}", "由「{0}」自动选择"),
    ("Profile.Why", "Why", "原因"),
...
FEATURES = [
    ...
    dict(id="OpenSshClient", name=("OpenSSH Client", "OpenSSH 客户端"), short=("Connect to remote servers over SSH from this PC", "在本机通过 SSH 连接远程服务器"), rec="OptionalRemove", risk="Low", action="Feature", mechanism="RemoveCapability", cat="Capability", scope="MountedImageFeature", restore="Easy", targets=[("OpenSSH.Client~~~~0.0.1.0", "Capability")], deps=[], keep=("sshclient-keep",), scen=("Developer", "RemoteDesktop"), tags=("ssh", "remote")),
    dict(id="OpenSshServer", name=("OpenSSH Server", "OpenSSH 服务器"), short=("Accept incoming SSH connections to this PC (opens a network port)", "接受指向本机的 SSH 连接（会开放网络端口）"), rec="AdvancedOnly", risk="Medium", action="Feature", mechanism="RemoveCapability", cat="Capability", scope="MountedImageFeature", restore="Easy", targets=[("OpenSSH.Server~~~~0.0.1.0", "Capability")], deps=[], keep=("sshserver-keep",), scen=("Developer", "EnterpriseDomain"), tags=("ssh", "remote")),
...
def emit_feature_cs():
    ...
    for f in FEATURES:
        ...
        lines.append("                Category = ComponentCategory.{},".format(f.get("cat", "OptionalFeature")))
