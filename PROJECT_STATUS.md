...
### Known Issues
- WIM index / edition / version / architecture / language parsing is implemented (Step 2.2) but the end-to-end mount → inspect → dismount → **metadata** cycle on a real Windows ISO is **not yet re-validated on a Windows desktop** after the DISM Error 87 fix — automated tests use fakes, and a regression test now guards the `/Get-ImageInfo` command. Until the user re-confirms the real Windows 11 25H2 zh-CN x64 Consumer `install.wim` metadata read with the corrected command, Step 2.2 stays NOT COMPLETED.
